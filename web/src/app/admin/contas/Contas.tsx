...
const CASA_COR: Record<string, string> = {
  ...
  BETANO: 'bg-amber-100 text-amber-800 border-amber-300', // cor da CASA (Betano), não da marca
};
...
export default function Contas({ contasIni }: { contasIni: Conta[] }) {
  ...
  const inp = 'w-full rounded-md border border-slate-300 bg-white px-2 py-1 text-sm text-slate-800 outline-none focus:border-marca-500';
  const numInp = 'w-24 rounded-md border border-slate-300 bg-white px-2 py-1 text-right text-sm tabular-nums text-slate-800 outline-none focus:border-marca-500';
    ...
    <main className="min-h-screen bg-slate-50 text-slate-800">
      <header className="bg-gradient-to-r from-[#13200a] to-[#1e2f10] text-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setNovo((n) => ({ ...n, open: !n.open }))} className="rounded-lg bg-marca-500 px-3 py-1.5 text-xs font-semibold text-white hover:bg-marca-600">+ Nova conta</button>
      ...
      <div className="mx-auto max-w-7xl px-4 py-5">
        ...
        <div className="mb-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          ...
          <div className="rounded-2xl border border-marca-100 bg-gradient-to-br from-marca-50 to-white p-4 shadow-sm">
            <div className="flex items-center gap-2">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-marca-100 text-lg">⚖️</span>
        ...
        {novo.open && (
          <div className="mb-4 rounded-xl border border-marca-200 bg-marca-50/60 p-4">
            ...
            <table className="w-full min-w-[980px] text-sm">
              ...
              <tbody>
                {g.rows.map((c) => {
                  ...
                  return (
                    <tr key={c.id} className={`border-b border-slate-100 align-middle ${emEdicao ? 'bg-marca-50/60' : ''}`}>
                      ...
                      {emEdicao ? (
                        <>
                          ...
                          <td className="px-2 py-1.5"><input title="Total acumulado. Para um novo depósito use “+ Lançar” — aqui só para corrigir." value={dv(c, 'deposito')} onChange={(ev) => upd(c.id, 'deposito', ev.target.value)} className={`${numInp} bg-marca-50`} inputMode="decimal" /></td>
                          <td className="px-2 py-1.5"><input title="Total acumulado. Para uma nova retirada use “+ Lançar” — aqui só para corrigir." value={dv(c, 'retirada')} onChange={(ev) => upd(c.id, 'retirada', ev.target.value)} className={`${numInp} bg-marca-50`} inputMode="decimal" /></td>
                        </>
                      ) : (
                        <>
                        ...
                        <div className="flex justify-center gap-1.5">
                          {emEdicao ? (
                            ...
                            <>
                              ...
                              <button onClick={() => startEdit(c.id)} title="Corrigir os dados da conta (saldo, em aberto, login…)" className="rounded-md bg-marca-500 px-2.5 py-1 text-xs font-medium text-white hover:bg-marca-600">Editar</button>
      ...
      {lanc && (() => {
        ...
        const inp = 'w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-marca-500';
        return (
                          ...
                          <td className="py-2 pr-2">
                            <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${
                              m.tipo === 'deposito' ? 'bg-blue-100 text-blue-700'
                                : m.tipo === 'retirada' ? 'bg-marca-100 text-marca-700'
