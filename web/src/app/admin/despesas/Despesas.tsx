...
import { listarDespesas, excluirDespesa } from '../actions';
...
export default function Despesas({ dadosIni }: { dadosIni: DespesasResp }) {
  ...
  const sem: SemanaDespesas = aba === 'atual' ? dados.atual : dados.passada;

  function recarregar() {
    startTransition(async () => { setDados(await listarDespesas()); });
  }
  function excluir(id: number) {
    if (!confirm('Excluir esta despesa?')) return;
    startTransition(async () => { await excluirDespesa(id); setDados(await listarDespesas()); });
  }
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-1 rounded-xl border border-slate-200 bg-white p-1">
            {(['atual', 'passada'] as const).map((k) => (
              <button key={k} onClick={() => setAba(k)} className={`rounded-lg px-4 py-1.5 text-sm font-medium ${aba === k ? 'bg-[#13200a] text-[#DAA520]' : 'text-slate-500 hover:text-slate-800'}`}>
                {k === 'atual' ? 'Semana atual' : 'Semana passada'}
              </button>
            ))}
            <span className="ml-2 pr-2 text-[11px] text-slate-400">{fmtDia(sem.d1)} a {fmtDia(sem.d2)}</span>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white px-4 py-2">
            <span className="text-[11px] uppercase tracking-wide text-slate-400">Total da semana</span>
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
          <table className="w-full min-w-[520px] text-sm">
            ...
            <tbody>
              ...
              {sem.rows.length === 0 && (
                <tr><td colSpan={4} className="px-3 py-10 text-center text-slate-400">Nenhuma despesa nesta semana.</td></tr>
              )}
            </tbody>
          </table>
        </div>
